...
float MuonTnP::getZSF(float pt1, float eta1, float pt2, float eta2, int idx){
  ...
  if(idx==1) return SF * ( 1 + TMath::Sqrt( idSF_varied + trigSF_varied + trackSF_varied ));//add relative uncertainties on the quantity, then add 1 and multiply by base value
  if(idx==-1) return SF * ( 1 - TMath::Sqrt( idSF_varied + trigSF_varied + trackSF_varied ));//add relative uncertainties on the quantity, then add 1 and multiply by base value
